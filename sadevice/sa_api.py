...
#Chemin vers sa_api.dll
salib = CDLL("C:\\Users\\LattePanda\\Desktop\\DesignIV\\sadevice\\sa_api.dll")
...
def sa_get_real_time_frame(device):
    ...
    sweep = numpy.zeros(sweep_length).astype(numpy.float32)
    ...
    status = saGetRealTimeFrame(device, sweep, color_frame, alpha_frame)
    return {
        "status": status,
        "sweep": sweep,
